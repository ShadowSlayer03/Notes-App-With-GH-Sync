...
<div bind:this={wrapper} class="relative">
	<button
		...
		class="flex h-12 w-full items-center justify-between rounded-xl border border-zinc-800 bg-[#181b22] px-4 transition hover:border-zinc-700"
	>
		<div class="flex items-center gap-3">
			<FolderIcon class="h-5 w-5 text-zinc-500" />
		...
		<div class={`transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
			<ChevronDown class="h-5 w-5 text-zinc-500" />
		</div>
	</button>
